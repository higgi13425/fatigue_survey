...
# Load data 
### had to fix to relative paths
```{r}
read.csv(here("MeasuringFatigueInIn-CDData111024_DATA_2024-11-11_0859.csv")) -> cd_data
...
```{r}
# make gender into an ordered factor, with self-encoding value labels
cd_data %>% 
  dplyr::mutate(gender = ordered(cd_gender,
                                   levels = c(1,2,3,4),
                                   labels = c(
                                     "1_Male",
                                     "2_Female",
                                     "3_Other",
                                     "4_Not_Disclosed"))) -> cd_gender

# check to confirm before proceeding
cd_gender %>% 
  tabyl(gender, cd_gender)
# all recoded accurately, all 1-3
...
```{r}
cd_gender %>%
  mutate(age = parse_number(cd_age)) ->
cd_age_df
...
cd_age_df %>% 
  filter(age<41) %>% 
  tabyl(age, cd_age)
# all recoded accurately, but also one "emptystring" for cd_age
...
cd_stop_bang_4 %>% 
  dplyr::mutate(bmi_sb = as_factor(cd_stop_bang5),
                bmi_sb = fct_recode(bmi_sb, 
                                      "0" = "0",
                                    "1" = "1", 
                                    "0" = "2"), 
                bmi_sb = relevel(bmi_sb, ref = "0")) -> cd_stop_bang_5
...
cd_stop_bang_8$bmi_sb <- as.integer(cd_stop_bang_8$bmi_sb) - 1
...
## Final CD data frame 
```{r}
cd_stop_bang_high %>% dplyr::select(sbj_id, ibd_dx, age, gender, race_6, race_2, ethnicity, fistula, stricture, abscess, surgery, hospital, severe_dx, active_steroids, prednisone_last_year, steroid_months_yr, steroid_month_life, meds_all, meds_mech, meds_class, psc, thyroid, anxiety, depression, osa, ibs, pro_bms, pro_liquid, pro_blood, pro_urgency, pro_abd_pain, pro_bloating, pro_joint_pain, pro_fatigue, pro_weak, pro_concentrate, pro_depressed, cd_bowel_domain, uc_bowel_domain, cd_functional_domain, uc_functional_domain, systemic_score, coping_score, daily_impact, emotional_impact, facit_1:facit_13,facit_total, facit_fatigue, ibd_f_total, snore_sb:male_sb, stop_bang_total, osa_mod_high, osa_high) -> cd_final 
